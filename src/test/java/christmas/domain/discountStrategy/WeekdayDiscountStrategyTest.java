...
class WeekdayDiscountStrategyTest {
    ...
    @BeforeEach
    void setup(){
        localDate = LocalDate.of(2023,12,7);
    ...
    void applyDiscount() {
        ...
        //when
        int result = 0;
        if (weekdayDiscountStrategy.isApplicable(localDate, order)){
            result = result = weekdayDiscountStrategy.calculateDiscount(localDate, order);
        }
        //then
